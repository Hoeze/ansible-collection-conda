...
This collection is tested weekly against ansible-core 2.19, 2.20 and 2.21, which are the currently supported ansible-core releases. Unreleased branches (`devel`, `milestone`) are not tested. `meta/runtime.yml` declares `requires_ansible: '>=2.19'` accordingly.

Integration tests cover only the oldest and newest target Python of each branch (3.8, 3.9, 3.13 and 3.14). Python 3.10 to 3.12 are supported but are not exercised by CI.
...
- `micromamba_version`: Currently has no effect. The role always installs the
  latest release from
...
See the [releases page](https://github.com/Hoeze/ansible-collection-conda/releases). From the
next release onwards, notes are also collected in `CHANGELOG.md`, generated by release-please
