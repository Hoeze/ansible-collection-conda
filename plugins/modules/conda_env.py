...
def conda_env_create_update(conda_exe, conda_env_subcommand, prefix, name, spec_file_path, dry_run=False):
    # Update environment
    cmd = [conda_exe, 'env', conda_env_subcommand, '-y', '--json', '--file', spec_file_path]
    if prefix:
        cmd.extend(['--prefix', prefix])
    if name:
        cmd.extend(['--name', name])
...
def main():
    ...
    try:
        ...
        if spec:
            ...
            conda_env_subcommand = "update" if env_check_result['is_valid_env'] else "create"
